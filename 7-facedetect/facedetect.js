// facedetect.js
// simple approach to detect skin colors and grow a rect region around it
...
function isSkin(r,g,b) {
  ...
  gn = g / base;
  bn = b / base;
  // try to simply detect skin colors
//  if (rn > 0.35 && rn < 0.5 && gn > 0.2 && gn < 0.5 && bn > 0.2 && bn < 0.35 && base > 250) {
  if (r > 150 && r < 250 && g > 110 && g < 190 && b > 80 && b < 160 && base > 250) {
    return true;
  } else {
    return false;
  }
  ...
  for (x = 0; x < width; x++) {
    for (y = 0; y < height; y++) {
      ...
      if (isSkin(frame.data[4*i],frame.data[4*i+1],frame.data[4*i+2])) {
        ...
        if(1) {
          // mark with a specific color
          frame.data[4*i+0] = 0;
          frame.data[4*i+1] = 200;
